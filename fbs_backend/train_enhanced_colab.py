...
======================================================
Run this in Google Colab to train the enhanced model

Steps:
1. Upload this script to Google Colab
2. Make sure Data_train.csv is in the same folder
3. Run all cells
4. Download flight_xgb_enhanced.pkl and feature_mapping_enhanced.json
5. Copy to fbs_backend folder
"""
...
for _, row in df.iterrows():
    ...
    # Business (1.5x - 2x)
    row_bus = row.copy()
    row_bus['Price'] = base_price * np.random.uniform(1.5, 2.0)
    ...
    row_first = row.copy()
    row_first['Price'] = base_price * np.random.uniform(2.5, 3.5)
...
print(f"After seat classes: {df_enhanced.shape}")
...
for _, row in df_enhanced.iterrows():
    ...
    for advance_days in [1, 3, 7, 14, 30, 60, 90]:
        if advance_days <= 3:
            price_factor = np.random.uniform(1.2, 1.5)
        elif advance_days <= 7:
...
mapping_path = 'feature_mapping_enhanced.json'
...
print("1. flight_xgb_enhanced.pkl")
print("2. feature_mapping_enhanced.json")
print("=" * 60)
