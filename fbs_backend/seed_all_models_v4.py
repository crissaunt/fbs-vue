...
def seed_data():
    ...
    print(f"✅ SeatClasses created for all airlines.")
    ...
    print("✅ Bookings, Payments, CheckIns: 20 each")

    # 12. Students & Instructors
    for i in range(20):
        Instructor.objects.create(instructor_id=f"INS{i:03d}", first_name=f"Ins{i}", last_name=f"Last{i}")
        u = User.objects.create_user(username=f"stu_user_{i}", email=f"student{i}@example.com", password="password")
        Students.objects.create(
            user=u, student_number=f"STU{i:03d}", first_name=f"Stu{i}", last_name=f"Last{i}",
